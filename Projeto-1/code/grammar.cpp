...
int take_state_name(char linha[], char state[], int i) {
  int j;
  for (i = next_simb(linha, i, '<') + 1, j = 0; linha[i] != '>'; i++)
    state[j++] = linha[i];
  state[j] = '\0';
  return i;
}
...
void mape_token
(FILE *input_file, int &states_cont, vpsb &states_name,
 map<string, int> &map_states, int &terms_cont, vs &terms_name,
 map<string, int> &map_terms, char linha[], int &states_qtty,
 vi &transition_cont, set<string>&initial_state_terms) {
  int i;
  char linha_tmp[2];
  linha_tmp[1] = '\0';
  if (!states_cont) {
  ...
  for (i = 0; linha[i] != '\n'; i++) {
    ...
    if (i == 0) {
      if (initial_state_terms.find(string(linha_tmp))
          == initial_state_terms.end()) {
...
void mape_grammar(FILE *input_file, viii &afnd) {
  int i, j, flag, id_current_state = 0;
  char state[MAX], term[MAX], linha[MAX];
  while (fgets(linha, MAX, input_file), next_simb(linha, 0, '<') != -1) {
    ...
    while (flag) {
      ...
      if (strcmp(term, "eps") && terminals_rule.find(string(term)) == terminals_rule.end()) {
        if (states_cont == 1) initial_state_terms.insert(string(term));
...
void symbols_mape(FILE *input_file, int &states_cont, vpsb &states_name, map<string, int> &map_states, int &terms_cont, vs &terms_name, map<string, int> &map_terms, int &states_qtty, vi &transition_cont, int &lim) {
  ...
  while (fgets(linha, MAX, input_file)) {
    ...
    if (next_simb(linha, 0, '<') == -1)
      mape_token(input_file, states_cont, states_name, map_states, terms_cont, terms_name, map_terms,
                 linha, states_qtty, transition_cont, initial_state_terms);
    else mape_grammar(input_file, states_cont, states_name, map_states, terms_cont, terms_name, map_terms,
                      linha, transition_cont, initial_state_terms, lim, states_qtty);
  }
